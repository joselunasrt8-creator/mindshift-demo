...
type TelemetryEventType = "SESSION_CREATED" | "CONTINUITY_CREATED" | "AUTHORITY_CREATED" | "AEO_COMPILED" | "VALIDATION_GRANTED" | "VALIDATION_REJECTED" | "EXECUTION_STARTED" | "EXECUTION_COMPLETED" | "PROOF_PERSISTED" | "PROOF_RECORDED" | "REPLAY_BLOCKED" | "HASH_MISMATCH" | "AUTHORITY_CONSUMED"
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/proof" && request.method === "POST") {
        ...
        const proofStatements = [
          env.DB.prepare(`INSERT OR IGNORE INTO proof_registry (proof_id,identity_id,session_id,continuity_id,continuity_hash,execution_id,decision_id,validated_object_hash,decision_hash,authority_lineage,execution_lineage,surface,run_id,commit_sha,workflow,environment,created_at,repository,branch,pull_request_id,merge_commit_sha,source_tree_hash,workflow_run_id,workflow_sha,delegated_authority_id,delegated_replay_chain_hash,delegation_lineage_hash,delegation_root_hash,parent_execution_hash,lineage_stage,lineage_origin_hash)
            ...
            WHERE a.decision_id=?4 AND a.session_id=?2 AND a.status='EXECUTED' AND c.status='ACTIVE' AND c.expires_at>?13
              ...
              AND EXISTS (SELECT 1 FROM execution_snapshot_registry x WHERE x.execution_id=?3 AND x.decision_id=?4 AND x.validated_object_hash=?5 AND x.invocation_nonce=?25 AND x.status='EXECUTED')
      ...
      await emitTelemetry(env, { event_type: "PROOF_RECORDED", decision_id, authority_id: String(authority.authority_id || ""), execution_id, proof_id, severity: "INFO", payload: { route: "/proof", session_id, validated_object_hash, repository: provenance.repository, branch: provenance.branch, workflow_run_id: provenance.workflow_run_id } })
