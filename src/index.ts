...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/execute" && request.method === "POST") {
      ...
      const validation = await env.DB.prepare(`SELECT * FROM validation_registry WHERE decision_id=?1 AND validated_object_hash=?2 AND invocation_nonce=?3 ORDER BY created_at DESC LIMIT 1`).bind(decision_id,validated_object_hash,invocation_nonce).first<any>()
      if (!validation) return rejectWithTelemetry(env, { status:"NULL", result:"INVALID", reason:"missing_validation" }, { event_type: "VALIDATION_REJECTED", decision_id, severity: "HIGH", payload: { route: "/execute", validated_object_hash, invocation_nonce, indicator: "validation_missing_before_execute" }, drift_class: "execution_drift" })
      if (String(validation.result || "") !== "VALID" || String(validation.status || "") !== "VALID") return rejectWithTelemetry(env, { status:"NULL", result:"INVALID", reason:"non_valid_validation" }, { event_type: "VALIDATION_REJECTED", decision_id, severity: "HIGH", payload: { route: "/execute", validated_object_hash, invocation_nonce, validation_result: String(validation.result || ""), validation_status: String(validation.status || ""), indicator: "validation_not_valid_before_execute" }, drift_class: "execution_drift" })
