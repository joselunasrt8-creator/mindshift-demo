...
test('no /execute path works without prior /validate evidence', () => {
  ...
  assert.match(source, /if \(!valid\) return rejectWithTelemetry\(env, \{ status:"NULL", result:"INVALID", reason:"no_validation" \}/)
  assert.match(source, /indicator: "execute_without_validate"/)
})
