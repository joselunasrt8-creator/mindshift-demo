...
test('federation reconciliation report route remains read-only and non-authoritative', () => {
  assert.match(source, /if \(url\.pathname === "\/federation\/reconcile\/report" && request\.method === "GET"\)/)
  assert.match(source, /federation_boundary: "portable_evidence_not_portable_authority"/)
  assert.match(source, /evidence_only: true/)
  assert.match(source, /replay_neutral: true/)
  assert.match(source, /read_only: true/)
  assert.match(source, /mutation_capable: false/)
...
test('reconciliation report continuity is lineage-bound and canonicalized', () => {
  assert.match(source, /const traversal_id = await sha256Hex\(canonicalize\(\{/)
  assert.match(source, /anchor: result\.lineage_anchor/)
  assert.match(source, /deterministic_traversal_trace: result\.deterministic_traversal_trace/)
  assert.match(source, /reconciliation_merkle_root: merkle\.root/)
  ...
  assert.match(source, /const quarantine_candidates = result\.drift_classifications/)
